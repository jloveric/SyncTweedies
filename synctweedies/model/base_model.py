...
from synctweedies.diffusion.controlnet import (
    SyncTweediesControlNet,
    SyncTweediesControlNetXL,
)
...
class BaseModel(metaclass=ABCMeta):
    ...
    def init_model(self, **kwargs):
        ...
        if self.config.model == "controlnet":
            ...
            pipe = StableDiffusionControlNetPipeline.from_pretrained(
                # "/home/loverich-j/RealisticVisionsHF",
                # "runwayml/stable-diffusion-v1-5",
                "stablediffusionapi/architecturerealmix",
        ...
        elif self.config.model == "controlnetxl":
            print("using controlnetxl")

            loras = {
                "xs": {
                    "path": "/data1/models/XSsdxl_1",
                    "name": "XSsdxl_1.safetensors",
                },
                "americana": {
                    "path": "/data1/models/AmericanaSDXL",
                    "name": "AmericanaSDXL.safetensors",
                },
                "zs": {
                    "path": "/data1/models/ZSprofiledV1.0-000058",
                    "name": "ZSprofiledV1.0-000058.safetensors",
                },
            }
            lora_name = "zs"
            ...
            # pipe = StableDiffusionXLControlNetPipeline.from_pretrained(
            #    "/data1/models/XSsdxl_1", controlnet=controlnet, vae=vae, torch_dtype=torch.float16
            # )
            pipe.load_lora_weights(
                loras[lora_name]["path"], weight_name=loras[lora_name]["name"]
            )
            # pipe = StableDiffusionControlNetPipeline.from_pretrained(
            # "runwayml/stable-diffusion-v1-5",
            #    "stablediffusionapi/architecturerealmix",
            ...
            pipe.scheduler = get_scheduler(pipe)
            """
            #print('pipe.components', pipe.components)
            self.model = SyncTweediesControlNetXL(
                gpu_id=self.config.gpu, 
                vae=pipe.components['vae'],
                text_encoder=pipe.components['text_encoder'],
                text_encoder_2=pipe.components['text_encoder_2'],
                tokenizer=pipe.components['tokenizer'],
                tokenizer_2=pipe.components['tokenizer_2'],
                unet=pipe.components['unet'],
                controlnet=pipe.components['controlnet'],
            ...
            pipe.components["unet"].config.addition_embed_type = None
        ...
        if self.config.model in ["sd", "controlnet", "controlnetxl"]:
    ...
    def compute_prev_state(self, xts, pred_x0s, timestep, eps, eta=0, **kwargs):
        """
        Input:
            xts: [N,C,H,W]
        Output:
        ...
        if self.config.sampling_method == "ddpm":
            ...
            xt_coeff = torch.sqrt(1 - alpha_prod_t_prev - std_dev_t**2) / torch.sqrt(
                1 - alpha_prod_t
            )
            x0_coeff = torch.sqrt(alpha_prod_t_prev) - torch.sqrt(
                alpha_prod_t
            ) / torch.sqrt(1 - alpha_prod_t) * torch.sqrt(
    ...
    def one_step_process(
        ...
        # Below are combined denoising processes.
            ...
            instance_delta_preds = self.inverse_mapping(
                instance_eps_preds, var_type="eps", **kwargs
            )
            instance_eps_preds = self.forward_mapping(
                instance_delta_preds, bg=instance_eps_preds, **kwargs
            )

            # canonical
            zts = input_params["zts"]
            xts_from_zts = self.forward_mapping(zts, **kwargs)
            canonical_eps_preds = self.compute_noise_preds(
                xts_from_zts, timestep, **kwargs
            )
            canonical_delta_preds = self.inverse_mapping(
                canonical_eps_preds, var_type="eps", **kwargs
            )
            canonical_eps_preds = self.forward_mapping(
                canonical_delta_preds, bg=canonical_eps_preds, **kwargs
            )
            ...
            x_t_1 = self.compute_prev_state(
                xts, x0s, timestep, combined_eps_preds, **kwargs
            )
            ...
            z_t_1 = self.compute_prev_state(
                zts, z0s, timestep, canonical_delta_preds, **kwargs
            )
            ...
            x_t_1 = self.compute_prev_state(
                xts, combined_x0s, timestep, instance_eps_preds, **kwargs
            )  # TODO: The use of instance_eps_preds would not be correct in DDPM sampling.

            # use canonical term only for canonical denoising.
            z_t_1 = self.compute_prev_state(
                zts, canonical_z0s, timestep, canonical_delta_preds, **kwargs
            )
            out_params = {
                "x0s": instance_x0s,
                "z0s": None,
            ...
            instance_z_t_1 = self.inverse_mapping(
                instance_x_t_1, var_type="latent", **kwargs
            )
            ...
            canonical_z_t_1 = self.compute_prev_state(
                zts, canonical_z0s, timestep, canonical_delta_preds, **kwargs
            )
